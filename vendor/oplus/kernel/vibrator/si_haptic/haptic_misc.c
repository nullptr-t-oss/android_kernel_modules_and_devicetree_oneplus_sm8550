...
int memo_file_open(struct inode *inode, struct file *file)
{
	...
	unsigned int cmd, unsigned long arg)
{
	...
	hp_info("%s: cmd:%u arg:%lu\n", __func__, cmd, arg);
	mutex_lock(&sih_haptic->lock);
	switch (cmd) {
	case IOCTL_GET_HWINFO:
		hp_info("%s:enter get hwinfo\n", __func__);
		tmp = IOCTL_HWINFO;
