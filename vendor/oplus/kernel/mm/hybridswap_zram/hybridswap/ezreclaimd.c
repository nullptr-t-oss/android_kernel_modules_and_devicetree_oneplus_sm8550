...
static isolate_page_t kp_isolate_page;
static struct tracepoint *tp_android_vh_si_mem_available_adjust;
static struct tracepoint *tp_android_vh_shrink_page_list;
static struct tracepoint *tp_android_vh_tune_swappiness;
static struct tracepoint *tp_android_vh_lru_gen_add_page_skip;
static struct tracepoint *tp_android_vh_lru_gen_del_page_skip;
static struct tracepoint *tp_android_rvh_perform_reclaim;
static struct tracepoint *tp_android_rvh_kswapd_shrink_node;
static struct tracepoint *tp_android_vh_clear_reclaimed_page;
static struct tracepoint *tp_android_vh_keep_reclaimed_page;
static struct tracepoint *tp_android_vh_mglru_should_abort_scan;
static struct tracepoint *tp_android_vh_evict_pages_bypass;
static struct tracepoint *tp_android_vh_pagecache_get_page;
static struct tracepoint *tp_android_vh_filemap_pages;
...
int __nocfi ezr_read_symbols_address(void)
{
	...
	/* memcg */
	kp_isolate_page =
		EZR_SYMBOL_LOOKUP(isolate_page);
	if (!kp_isolate_page)
		return -EINVAL;
	kp_mem_cgroup_iter =
	...
	tp_android_vh_si_mem_available_adjust =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_si_mem_available_adjust);
	if (!tp_android_vh_si_mem_available_adjust)
		return -EINVAL;
	tp_android_vh_shrink_page_list =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_shrink_page_list);
	if (!tp_android_vh_shrink_page_list)
		return -EINVAL;
	tp_android_vh_tune_swappiness =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_tune_swappiness);
	if (!tp_android_vh_tune_swappiness)
		return -EINVAL;

	/* mglru hook */
	tp_android_vh_lru_gen_add_page_skip =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_lru_gen_add_page_skip);
	if (!tp_android_vh_lru_gen_add_page_skip)
		return -EINVAL;
	tp_android_vh_lru_gen_del_page_skip =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_lru_gen_del_page_skip);
	if (!tp_android_vh_lru_gen_del_page_skip)
		return -EINVAL;

	/* reclaim hook */
	tp_android_rvh_perform_reclaim =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_rvh_perform_reclaim);
	if (!tp_android_rvh_perform_reclaim)
		return -EINVAL;
	tp_android_rvh_kswapd_shrink_node =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_rvh_kswapd_shrink_node);
	if (!tp_android_rvh_kswapd_shrink_node)
		return -EINVAL;
	tp_android_vh_clear_reclaimed_page =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_clear_reclaimed_page);
	if (!tp_android_vh_clear_reclaimed_page)
		return -EINVAL;

	/* demote hook */
	tp_android_vh_keep_reclaimed_page =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_keep_reclaimed_page);
	if (!tp_android_vh_keep_reclaimed_page)
		return -EINVAL;
	tp_android_vh_mglru_should_abort_scan =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_mglru_should_abort_scan);
	if (!tp_android_vh_mglru_should_abort_scan)
		return -EINVAL;
	tp_android_vh_evict_pages_bypass =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_evict_pages_bypass);
	if (!tp_android_vh_evict_pages_bypass)
		return -EINVAL;

	/* filemap hook */
	tp_android_vh_pagecache_get_page =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_pagecache_get_page);
	if (!tp_android_vh_pagecache_get_page)
		return -EINVAL;
	tp_android_vh_filemap_pages =
		EZR_SYMBOL_LOOKUP(__tracepoint_android_vh_filemap_pages);
	if (!tp_android_vh_filemap_pages)
		return -EINVAL;
				      ...
				      struct list_head *list)
{
	...
	while (!list_empty(head)) {
		...
		if (sort_ezreclaimable_page(lruvec, page))
			sorted += delta;
		else if (kp_isolate_page(lruvec, page, sc)) {
...
static int __nocfi register_vendor_hooks(void)
{
	...
	ret = tracepoint_probe_register(tp_android_vh_si_mem_available_adjust,
					ezr_vh_available_adjust, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_shrink_page_list,
					ezr_vh_shrink_page_list, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_tune_swappiness,
					ezr_vh_tune_swappiness, NULL);
	if (ret)
		return ret;

	/* mglru hook */
	ret = tracepoint_probe_register(tp_android_vh_lru_gen_add_page_skip,
					ezr_vh_lru_gen_add_page_skip, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_lru_gen_del_page_skip,
					ezr_vh_lru_gen_del_page_skip, NULL);
	if (ret)
		return ret;

	/* reclaim hook */
	ret = tracepoint_probe_register(tp_android_rvh_perform_reclaim,
					ezr_rvh_perform_reclaim, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_rvh_kswapd_shrink_node,
					ezr_rvh_kswapd_shrink_node, NULL);
	if (ret)
		return ret;
	ret = register_trace_android_vh_look_around_migrate_page(ezr_vh_look_around_migrate_page, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_clear_reclaimed_page,
					ezr_vh_clear_reclaimed_page, NULL);
	if (ret)
		return ret;
	...
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_keep_reclaimed_page,
					ezr_vh_keep_reclaimed_page, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_mglru_should_abort_scan,
					ezr_vh_mglru_should_abort_scan, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_evict_pages_bypass,
					ezr_vh_evict_pages_bypass, NULL);
	if (ret)
		return ret;

	/* filemap hook */
	ret = tracepoint_probe_register(tp_android_vh_pagecache_get_page,
					ezr_vh_filemap_get_page, NULL);
	if (ret)
		return ret;
	ret = tracepoint_probe_register(tp_android_vh_filemap_pages,
					ezr_vh_filemap_pages, NULL);
	if (ret)
		return ret;
	return ret;
}
...
static int ezreclaimable_init(void)
{
	...
	/* for kernel-5.15, use function pointer instead of vendor_hook */
